...
def main():
    ...
    # Update overall stats at the beginning of every render
    ...
    with st.sidebar:
        ...
        # Get fresh stats every time to ensure updates are reflected
        stats = st.session_state.overall_stats
        
        # Display metrics with proper formatting
        col1, col2 = st.columns(2)
        with col1:
            st.metric("Completed", f"{stats['completed']}/15")
        with col2:
            st.metric("Passed", f"{stats['passed']}/15")
        ...
        if stats["total_questions"] > 0:
            ...
            st.metric("Overall Score", "0/0 (0.0%)")
    ...
    if st.session_state.current_section is None:
        ...
        if not st.session_state.test_completed:
            ...
            # Update overall stats immediately
            update_overall_stats()
            
            # Force sidebar to refresh by updating a counter
